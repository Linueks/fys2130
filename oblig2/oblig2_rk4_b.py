...
mass = 0.1                                                      # [kg]
spring_coeff = 10                                               # [N/m]
drag_coeff = 0.040                                              # [kg/s]
omega_0 = np.sqrt(spring_coeff / mass)                          # [s^-1]
periodic_force = 0.1                                            # [N]
force_frequency = omega_0                                              # [s^-1]
g = const.g                                                     # [m/s^2]
...
"""
alpha = -gamma +- sqrt(gamma^2 - omega^2)

over critical dampening: gamma > omega -> b>2*sqrt(k*m)

critical dampening: gamma = omega b = 2*sqrt(k*m)

under critical dampening: gamma < omega -> b<2*sqrt(k*m)
"""
...
def spring_pendulum(pos, vel, t):
    ...
    F_periodic = periodic_force * np.cos(force_frequency * t * dt)


    acceleration[:, t] = (F_gravity + F_spring + F_drag + F_periodic) / mass
...
def integrator_loop(plot=True):
    ...
    if plot:
        ...
        plt.title('Mechanical Spring Pendulum, Periodic Force')
        plt.show()
